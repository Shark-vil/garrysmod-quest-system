...
if SERVER then
	...
	function QuestService:StartNPCRandomWalk(npc)
		if IsValid(npc) and npc:IsNPC() and not table.HasValue(self.npcs_random_walk, npc) then
	...
	function QuestService:StopNPCRandomWalk(npc)
		if IsValid(npc) and npc:IsNPC() and table.HasValue(self.npcs_random_walk, npc) then
...
function QuestService:PlayerIsViewVector(ply, pos)
	...
	if IsView then return true end
