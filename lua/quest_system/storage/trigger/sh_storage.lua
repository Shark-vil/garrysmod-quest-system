...
if SERVER then
	...
	util.AddNetworkString('sv_qsystem_trigger_read')
...
function storage:GetFilePath(id, name)
	local file_path = 'quest_system/triggers/' .. id .. '/' .. game.GetMap() .. '/' .. name .. '.json'
	return file_path
end
...
if SERVER then
	net.Receive('sv_qsystem_trigger_read', function(len, ply)
		if not ply:IsQuestEditAccess() then return end
		local id = net.ReadString()
		local name = net.ReadString()
		...
		if fileData ~= nil then
			snet.Invoke('triggers_read' .. name, ply, fileData)
		end
	end)
end
...
function storage:Read(id, name, callback)
	if CLIENT then
		if callback ~= nil and isfunction(callback) then
			snet.RegisterCallback('triggers_read' .. name, callback, true, true)
		...
		if id ~= nil and string.len(id) ~= 0 and name ~= nil and string.len(name) ~= 0 then
...
QuestSystem:SetStorage('trigger', storage)