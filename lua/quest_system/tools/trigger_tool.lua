...
if SERVER then
    util.AddNetworkString('network_qsystem_open_trigger_editor')
    util.AddNetworkString('network_qsystem_close_trigger_editor')

    net.Receive('network_qsystem_open_trigger_editor', function(len, ply)
        if not ply:Alive() then return end

        ply:Give(weapon_class)
        ply:SelectWeapon(weapon_class)
        net.Start('network_qsystem_open_trigger_editor')
        net.Send(ply)
    end)

    net.Receive('network_qsystem_close_trigger_editor', function(len, ply)
    ...
    net.Receive('network_qsystem_open_trigger_editor', function(len, ply)
        ...
        InfoButtonNo.DoClick = function()
            net.Start('network_qsystem_close_trigger_editor')
            net.SendToServer()
            InfoPanel:Close()
        end
